...
if [ -z "$CMD_NAME" ]; then
    if [ -t 0 ]; then
        ...
        echo "Error: --cmd parameter is required when running non-interactively"
        exit 1
    fi
fi
...
if [ -z "$BASE_URL" ]; then
    if [ -t 0 ]; then
        ...
        echo "Error: --base-url parameter is required when running non-interactively"
        exit 1
    fi
fi

# Ensure values are provided after prompting
